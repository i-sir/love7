    ...
    public function index()
    {
        ...
        $users = UserModel::where('user_type', 1)
            ...
            ->each(function ($item, $key) {
                //角色身份
                $role_list = Db::name('role_user')->where('user_id', $item['id'])->select();
                $role_ids  = $role_list->column('role_id');

                $role_name         = Db::name('role')->where('id', 'in', $role_ids)->column('name');
                $item['role_name'] = implode(',', $role_name);
