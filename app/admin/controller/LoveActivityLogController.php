 ...
 *     "controller_name"     =>"LoveActivityLog",
 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\LoveActivityLogController();
 * )
 */
...
class LoveActivityLogController extends AdminBaseController
{
...
//    }
    ...
    protected function base_index()
    {
        ...
        $this->assign("activity_list", $LoveActivityInit->get_list());

        //状态
        $LoveActivityLogInit = new \init\LoveActivityLogInit();//报名管理    (ps:InitController)
        $this->assign("status_list", $LoveActivityLogInit->status);
    ...
    public function index()
    {
        $this->base_index();
        $LoveActivityLogInit  = new \init\LoveActivityLogInit();//报名管理    (ps:InitController)
        $LoveActivityLogModel = new \initmodel\LoveActivityLogModel(); //报名管理   (ps:InitModel)
        $params               = $this->request->param();

        //查询条件
        $where = [];
        if ($params["keyword"]) $where[] = ["username|phone", "like", "%{$params["keyword"]}%"];
        ...
        //查询数据
        $result = $LoveActivityLogInit->get_list_paginate($where, $params);
    ...
    public function edit()
    {
        ...
        //查询数据
        $params["InterfaceType"] = "admin";//接口类型
        $result                  = $LoveActivityLogInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        //提交数据
        $result = $LoveActivityLogInit->admin_edit_post($params, $where);
    ...
    public function edit_post_two()
    {
        ...
        //提交数据
        $result = $LoveActivityLogInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        if ($params['status'] == 2) $params['reject_time'] = time();
        if ($params['status'] == 3) $params['pass_time'] = time();
    ...
    public function add_post()
    {
        ...
        //插入数据
        $result = $LoveActivityLogInit->admin_edit_post($params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function delete()
    {
        ...
        //删除数据
        $result = $LoveActivityLogInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $LoveActivityLogInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $LoveActivityLogInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $LoveActivityLogInit->get_list($where, $params);
