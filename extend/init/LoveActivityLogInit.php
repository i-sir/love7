 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\LoveActivityLogInit();
 * )
 */
...
class LoveActivityLogInit extends Base
{

    public $status = [1 => '审核中', 2 => '未通过', 3 => '已通过'];//状态
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        if ($this->InterfaceType == 'api') {
            ...
            //活动名字
            $activity_info         = $LoveActivityModel->where('id', '=', $item['activity_id'])->find();
    ...
    public function get_join_list($where = [], $params = [])
    {
        ...
        $result = $LoveActivityLogModel
            ->alias('l')
            ->join('love_activity a', 'l.activity_id = a.id')
            ->where($where)
            ->order($params['order'] ?? $this->Order)
            ->field($params['field'] ?? $this->Field)
            ->paginate(["list_rows" => $params["page_size"] ?? $this->PageSize, "query" => $params])
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $LoveActivityLogModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $LoveActivityLogModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $LoveActivityLogModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $LoveActivityLogModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $LoveActivityLogModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $LoveActivityLogModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $LoveActivityLogModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
