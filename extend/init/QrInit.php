...
class QrInit
{
     ...
     * @param string $poster_text 介绍
     */
    public function applet_share($pathname = '',$poster_text='')
    {
        //仓库地址
        $storage      = cmf_get_option('storage');
        $pluginClass  = cmf_get_plugin_class($storage['type']);
        ...
        $new_path = app()->getRootPath() . "public/upload/{$dz}/{$qr_path}/{$date}/" . cmf_random_string(45) . ".png";
        $eq_img   = Image::open($new_pathname);//短路径
        $eq_img->thumb(450, 450, 2)->save($new_path);//将二维码压缩大小


        //绘画  写logo 写文字
        $img = Image::open($images_path);
        $img->water($new_path, [145, 720])->save($images_path);//背景图+小程序码(已压缩大小)

        //写文字
        $font = app()->getRootPath() . "public/font/song.otf"; //字体在服务器上的绝对路径
        $img->text($poster_text, $font, 20, '#000000', [100, 1380])->save($images_path);//生成文字
