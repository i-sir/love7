 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\MemberAuthenticationListInit();
 * )
 */
...
class MemberAuthenticationListInit extends Base
{
    public $type = [1 => '手机认证', 2 => '单身认证', 3 => '身份认证', 4 => '购房认证', 5 => '购车认证', 6 => '收入认证', 7 => '学历认证', 8 => '健康认证', 9 => '单位认证'];//认证类型
    ...
    public function _init()
    {
        ...
        $MemberAuthenticationListModel = new \initmodel\MemberAuthenticationListModel(); //认证列表  (ps:InitModel)
    }
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //处理转文字
        $item['type_name'] = $this->type[$item['type']];//认证类型
        ...
        if ($this->InterfaceType == 'api') {
            ...
            if ($item['select_image']) $item['select_image'] = cmf_get_asset_url($item['select_image']);
    ...
    public function get_list($where = [], $params = [])
    {
        ...
        //查询数据
        $result = $MemberAuthenticationListModel
            ->where($where)
    ...
    public function get_find($where = [], $params = [])
    {
        ...
        //查询数据
        $item = $MemberAuthenticationListModel
            ->where($where)
            ->order($params['order'] ?? $this->Order)
            ->field($params['field'] ?? $this->Field)
            ->find();
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $MemberAuthenticationListModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $MemberAuthenticationListModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $MemberAuthenticationListModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $MemberAuthenticationListModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $MemberAuthenticationListModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $MemberAuthenticationListModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $MemberAuthenticationListModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
