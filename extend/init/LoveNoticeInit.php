...
class LoveNoticeInit extends Base
{
    public $type   = [1 => '见面邀请', 2 => '系统通知'];//类型
    public $status = [1 => '审核中', 2 => '已同意', 3 => '已拒绝'];//状态
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //查询用户信息
        $user_info         = $MemberInit->get_find(['id' => $item['user_id']],['is_authentication'=>true]);
        $item['user_info'] = $user_info;

        //查询用户信息2
        $to_user_info         = $MemberInit->get_find(['id' => $item['to_user_id']],['is_authentication'=>true]);
        $item['to_user_info'] = $to_user_info;

        //显示用户
        if ($params['user_id'] == $item['user_id']) $item['show_user_info'] = $to_user_info;
        if ($params['user_id'] == $item['to_user_id']) $item['show_user_info'] = $user_info;
        ...
        if ($this->InterfaceType == 'api') {
            ...
            //admin处理文件
            $map               = [];
            $map[]             = ['id', 'in', $this->getParams($item['send_user_id'])];
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $LoveNoticeModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $LoveNoticeModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $LoveNoticeModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $LoveNoticeModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $LoveNoticeModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $LoveNoticeModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $LoveNoticeModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
