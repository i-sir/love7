 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\MemberRecommendOrderInit();
 * )
 */
...
class MemberRecommendOrderInit extends Base
{
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //查询用户信息
        $user_info         = $MemberRecommendInit->get_find(['id' => $item['user_id']]);
    ...
    public function get_list($where = [], $params = [])
    {
        ...
        //查询数据
        $result = $MemberRecommendOrderModel
            ->where($where)
    ...
    public function get_find($where = [], $params = [])
    {
        ...
        //查询数据
        $item = $MemberRecommendOrderModel
            ->where($where)
            ->order($params['order'] ?? $this->Order)
            ->field($params['field'] ?? $this->Field)
            ->find();
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $MemberRecommendOrderModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $MemberRecommendOrderModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $MemberRecommendOrderModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $MemberRecommendOrderModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $MemberRecommendOrderModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $MemberRecommendOrderModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $MemberRecommendOrderModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
