...
class MemberInit extends Base
{
    ...
    public $status        = [1 => '审核中', 2 => '已通过', 3 => '已驳回'];
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //截取年
        $item['birth_name'] = substr($item['birth'], 0, 4);

        //头像
        $item['is_avatar'] = false;
        if ($item['vip_id'] >= 2) $item['is_avatar'] = true;
        ...
        //爱好
        if ($item['hobby']) $item['hobby'] = $this->getParams($item['hobby']);
        //标签
        ...
        if ($this->InterfaceType == 'api') {
            ...
            //视频
            if ($item['video']) $item['video'] = $this->getImagesUrl($item['video']);
            if (empty($item['video'])) $item['video'] = [];

            //认证列表
            if (empty($params['is_authentication'])) {
                $auth_list           = $MemberAuthenticationListInit->get_list();
                $authentication_list = [];
                foreach ($auth_list as $key => $value) {
                    $map     = [];
                    $map[]   = ['type', '=', $value['type']];
                    $map[]   = ['status', '=', 2];
                    $is_auth = $MemberAuthenticationModel->where($map)->find();
                    if ($is_auth) {
                        $value['true_image']   = cmf_get_asset_url($value['true_image']);
                        $authentication_list[] = $value;
                    }
                }
                $item['authentication_list'] = $authentication_list;
            }
            ...
            //视频
            if ($item['video']) $item['video'] = $this->getParams($item['video']);
        }
