 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\MemberRecommendInit();
 * )
 */
...
class MemberRecommendInit extends Base
{

    public $status   = [1 => '审核中', 2 => '已通过', 3 => '已拒绝'];//状态
    public $is_admin = [1 => '未充值', 2 => '正常', 3 => '已到期'];//管理
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        $item['is_admin_name'] = $this->is_admin[$item['is_admin']];//管理

        //头像
        $item['is_avatar'] = true;
        //if ($item['is_manage'] == 2) $item['is_avatar'] = true;
        ...
        if ($this->InterfaceType == 'api') {
            ...
            if ($item['license_images']) $item['license_images'] = $this->getImagesUrl($item['license_images']);
            if ($item['avatar']) $item['avatar'] = cmf_get_asset_url($item['avatar']);


        } else {
            //admin处理文件
            if ($item['license_images']) $item['license_images'] = $this->getParams($item['license_images']);
        }
    ...
    public function api_edit_post($params = [], $where = [])
    {
        ...
        //处理共同数据
        if ($params['license_images']) $params['license_images'] = $this->setParams($params['license_images']);
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $MemberRecommendModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $MemberRecommendModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $MemberRecommendModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $MemberRecommendModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $MemberRecommendModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $MemberRecommendModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $MemberRecommendModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
