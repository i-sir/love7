...
HTML;
    ...
    public function get_index_html($type, $field, $fields_value, $i, $f)
    {
        ...
        if ($type == 'image') {
            ...
            $result    .= "<img src='{:cmf_get_asset_url($f_i)}' style='width: 40px;'>" . "\n";
            $result    .= '</a>' . "\n";
            $result    .= '<else/>' . "\n";
            $result    .= '暂无上传' . "\n";
            $result    .= '</if>' . "\n";
        ...
        if ($type == 'user_id') {
            ...
            $result       .= "<img src='{:cmf_get_asset_url($avatar)}' style='width: 40px;'>" . "\n";
            $result       .= '</a>' . "\n";
...
EOF;
    ...
    public function indexViews($parm, $controllers, $fields, $table_result_name)
    {
        ...
        $this->assign("url_edit", "{:url('edit',['id'=>\$vo.id,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}");
        $this->assign("url_edit_id", "{:url('edit',['id'=>\$id,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}");//编辑id
        $this->assign("url_delete_all", "{:url('delete',['page'=>\$page,'pid'=>\$pid,'params'=>\$params])}");


        $delete_id = "{:url('delete',['id'=>\$vo.id,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}";
        $find_id   = "{:url('find',['id'=>\$vo.id,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}";
        $edit_id   = "{:url('edit',['id'=>\$vo.id,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}";
        $find_name = "{\$vo.name}";
        ...
        $this->assign("recommend_post", "{:url('batch_post',['is_recommend'=>2,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}");
        $this->assign("no_recommend_post", "{:url('batch_post',['is_recommend'=>1,'page'=>\$page,'pid'=>\$pid,'params'=>\$params])}");
