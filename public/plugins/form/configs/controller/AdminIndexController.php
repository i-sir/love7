...
class AdminIndexController extends PluginAdminBaseController
{
    ...
    protected function initialize()
    {
        ...
        //本地测试展示内容
        if ($CUSTODIAN_SIR) {
            $list = Db::name('base_config')->where('is_menu', 'in', [1, 3])->order('list_order,key,id desc')->select()->toArray();
            foreach ($list as $value) {
                $group_array[$value['key']] = $value['menu_name'];
            }
            $this->group_list = $group_array;
    ...
    private function saveConfig($group_id)
    {
        ...
        //处理显示隐藏
        if ($params['is_show']) {
            $show_list = $this->request->param('show');
            if ($show_list) {
                foreach ($show_list as $k => $v) {
                    Db::name('base_config')->where(['name' => $k])->update(['is_show' => 1]);
                }
            }
        }
