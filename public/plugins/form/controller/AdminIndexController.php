...
HTML;
    ...
    public function settingPost()
    {
        ...
        $AdminMenuParams['controller'] = $parm['model_name'];
        ...
        if (isset($parm['api_controller']) && $parm['api_controller']) {
            $controller_content = $this->ApiControllerViews($parm, $controllers, $fields, $table_result_name, $keyword);//api控制器
        ...
        if (isset($parm['admin_controller']) && $parm['admin_controller']) {
            $controller_content = $this->AdminControllerViews($parm, $controllers, $fields, $keyword, $table_result_name);
        ...
        //init 公共模型Model
        $initmodel_content = $this->InitModelViews($parm, $controllers, $fields, $table_result_name);
        ...
        if (isset($controllers) && $controllers) {
            $controller_content = $this->InitViews($controllers, $parm, $table_result_name, $fields);
        ...
        if (isset($parm['admin_view']) && $parm['admin_view']) {
            $this->model->template_View($parm,
                $this->indexViews($parm, $controllers, $fields, $table_result_name),
                $this->addViews($parm, $controllers, $fields, $table_result_name),
                $this->editViews($parm, $controllers, $fields, $table_result_name),
    ...
    public function translate($value = 'undefined')
    {
        ...
        if (empty($result['trans_result'])) return '';
    ...
    public function get_edit_html($type, $field, $fields_value, $i, $f)
    {
        ...
        if ($type == 'images') {
            ...
            $saved  = "$('#saved-goods-image{$key}')";
            $saved  = "{{$saved}remove();}";
            $remove = "<a href='javascript:(function()$saved)();'>移除</a>";

            $result .= "<if condition='!empty($f_i)'>" . "\n";
            $result .= "<ul id='photos-goods-$i' class='pic-list list-unstyled form-inline'>" . "\n";
            $result .= "<foreach name='$f' item='vo'>" . "\n";
            $result .= "<li id='saved-goods-image$key'>" . "\n";
            $result .= "<div style='float:left; width:90px;'>" . "\n";
            $result .= "<input id='photo-goods-$key' type='hidden' name='images[]' value='$vo ='>" . "\n";
            $result .= "<img id='photo-goods-$key-preview' src='{:cmf_get_image_preview_url($vos)}' width='80' onclick='parent.imagePreviewDialog(this.src);'>" . "\n";
            $result .= '<a href="javascript:uploadOneImage(' . "'图片上传'" . ',' . "#photo-goods-$key" . ');">替换</a>' . "\n";
            $result .= $remove . "\n";
            $result .= "</div>" . "\n";
            $result .= "</li>" . "\n";
            $result .= "</foreach>" . "\n";
            $result .= "</ul>" . "\n";
            $result .= "<else/>" . "\n";
            $result .= "<ul id='photos-goods' class='pic-list list-unstyled form-inline'>" . "\n";
            $result .= "<li id='saved-goods-image'>" . "\n";
            $result .= "<div style='float:left; width:90px;'>" . "\n";
            $result .= "</div>" . "\n";
            $result .= "</li>" . "\n";
            $result .= "</ul>" . "\n";
            $result .= "</if>" . "\n";
            $result .= '<a  class="btn btn-sm btn-default" href="javascript:uploadOneImage(' . "'图片上传'" . ',' . "'#photos-goods'" . ',' . "photos-goods-item-tpl-$i" . '); ">选择图片</a>' . "\n";

            $result .= "<script type='text/html' id='photos-goods-item-tpl-$i'>
            <li id='saved-goods-image{id}'>
                <div style='float:left; width:90px;'>
                    <input id='photo-goods-{id}' type='hidden' name='images[]' value='{filepath}'>
        ...
        if ($type == 'image') {
            ...
            $result .= "<img src='{:cmf_get_image_preview_url($f_i)}' id='photo-000000$i-preview' style='width: 85px;height: 80px'>" . "\n";
            $result .= '</a>' . "\n";
            $result .= '<else/>' . "\n";
            $result .= "<img src='/static/images/default.png' id='photo-000000$i-preview' style='width: 85px;height: 80px'>" . "\n";
            $result .= '</if>' . "\n";
    ...
    public function get_index_html($type, $field, $fields_value, $i, $f)
    {
        ...
        if ($type == 'image') {
            ...
            $result    .= "<img src='{:cmf_get_asset_url($f_i)}' style='width: 40px;height: 45px'>" . "\n";
            $result    .= '</a>' . "\n";
            $result    .= '<else/>' . "\n";
            $result    .= '暂无上传' . "\n";
            $result    .= '</if>' . "\n";
        ...
        if ($type == 'user_id') {
            ...
            $result       .= "<img src='{:cmf_get_asset_url($avatar)}' style='width: 40px;height: 40px;'>" . "\n";
            $result       .= '</a>' . "\n";
...
EOF;
     ...
     */
    public function AdminControllerViews($parm, $controllers, $fields, $keyword = 'id', $table_result_name)
    {
...
EOF;
        ...
        $content = str_replace(
            ['%InitName%', '%name_underline%', '%InitModel%', '%root_menu_name%', '%menu%', '%menu_parent%', '%controller_name%', '%model_name%', '%keyword%', '%table_result_name%', '%get_list%', '%ValidateName%'],
            [$InitName, $name_underline, $InitModel, $root_menu_name, $parm['menu'], $parm['menu_parent'], $parm['controllers'], $parm['model_name'], $keyword, $table_result_name, $get_list, $ValidateName],
            $content
        );
...
EOF;
    ...
    /**
     * init公共配置 渲染代码
     * @param $controllers init名字
     * @return array|false|string|string[]
     */
    public function InitViews($controllers, $parm, $table_result_name, $fields)
    {
...
EOF;
        ...
        foreach ($fields as $k => $v) {
            if (!in_array($v['COLUMN_NAME'], $filtration_array)) {
                ...
                if ($v['type'] == 'images') {
                    //列表图片处理
                    $find_admin_images .= "if(\$item['{$name}']) \$item['{$name}']=\$this->getParams(\$item['{$name}']);";
                    $find_images       .= "if(\$item['{$name}']) \$item['{$name}']=\$this->getImagesUrl(\$item['{$name}']);";
                    $find_images_list  .= "if(\$item['{$name}']) \$item['{$name}']=\$this->getImagesUrl(\$item['{$name}']);";
                    $edit_images       .= "if(\$params['{$name}']) \$params['{$name}']=\$this->setParams(\$params['{$name}']);";
                }
                ...
                if ($v['type'] == 'image') {
                    ...
                    $find_image_list .= "if(\$item['{$name}']) \$item['{$name}']=cmf_get_asset_url(\$item['{$name}']);";
                }


                if ($v['type'] == 'video') $find_video .= "if(\$item['{$name}']) \$item['{$name}']=cmf_get_asset_url(\$item['{$name}']);";


                if ($v['type'] == 'file') $find_file .= "if(\$item['{$name}']) \$item['{$name}']=cmf_get_asset_url(\$item['{$name}']);";


                //富文本
                if ($v['type'] == 'content') $find_content .= "if(\$item['{$name}']) \$item['{$name}']=htmlspecialchars_decode(cmf_replace_content_file_url(\$item['{$name}']));";
        ...
        //处理数据源-InitModel
        $content = str_replace(
            ['%OrderField%', '%InitName%', '%use%', '%member_model%', '%user_info%', '%user_info_list%', '%InitModel%', '%root_menu_name%', '%find_images_list%', '%find_image_list%', '%find_image%', '%find_video%', '%find_file%', '%find_content%', '%edit_images%', '%find_images%', '%find_admin_images%', '%table_result_name%', '%init_field%', '%init_field_name%', '%ModelName%'],
            [$order_field, $InitName, $use, $member_model, $user_info, $user_info_list, $InitModel, $root_menu_name, $find_images_list, $find_image_list, $find_image, $find_video, $find_file, $find_content, $edit_images, $find_images, $find_admin_images, $table_result_name, $parm['init_field'], $parm['init_field_name'], $ModelName],
            $content
        );
     ...
     * @return array|false|string|string[]
     */
    public function InitModelViews($parm, $controllers, $fields, $table_result_name)
    {
...
EOF;
     ...
     * @throws \Exception
     */
    public function indexViews($parm, $controllers, $fields, $table_result_name)
    {
        ...
        $this->assign("url_list", "{:url('index',['params'=>\$params])}");
        $this->assign("url_add", "{:url('add',['params'=>\$params])}");
        $this->assign("url_edit", "{:url('edit',['id'=>\$vo.id,'page'=>\$page,'params'=>\$params])}");
        $this->assign("url_edit_id", "{:url('edit',['id'=>\$id,'page'=>\$page,'params'=>\$params])}");//编辑id
        $this->assign("url_delete_all", "{:url('delete',['page'=>\$page,'params'=>\$params])}");


        $delete_id = "{:url('delete',['id'=>\$vo.id,'page'=>\$page,'params'=>\$params])}";
        $find_id   = "{:url('find',['id'=>\$vo.id,'page'=>\$page,'params'=>\$params])}";
        $edit_id   = "{:url('edit',['id'=>\$vo.id,'page'=>\$page,'params'=>\$params])}";
        $find_name = "{\$vo.name}";
        ...
        $this->assign("recommend_post", "{:url('batch_post',['is_recommend'=>2,'page'=>\$page,'params'=>\$params])}");
        $this->assign("no_recommend_post", "{:url('batch_post',['is_recommend'=>1,'page'=>\$page,'params'=>\$params])}");
        $this->assign("list_order_post", "{:url('list_order_post',['page'=>\$page,'params'=>\$params])}");
    ...
    protected function addViews($parm, $controllers, $fields, $table_result_name)
    {
        ...
        $this->assign("url_list", "{:url('index',['page'=>\$page,'params'=>\$params])}");
        $this->assign("url_add", "{:url('add',['page'=>\$page,'params'=>\$params])}");
        $this->assign("url_add_post", "{:url('add_post',['page'=>\$page,'params'=>\$params])}");
        $this->assign("url_edit_id", "{:url('edit',['id'=>\$id,'page'=>\$page,'params'=>\$params])}");//编辑id
        $this->assign("admin_js", "__STATIC__/js/admin.js");//引入admin.js文件
    ...
    protected function editViews($parm, $controllers, $fields, $table_result_name)
    {
        ...
        $this->assign("url_add", "{:url('add',['page'=>\$page,'params'=>\$params])}");
        $this->assign("url_edit_id", "{:url('edit',['id'=>\$id,'page'=>\$page,'params'=>\$params])}");//编辑id
        $this->assign("url_edit_post", "{:url('edit_post',['id'=>\$id,'page'=>\$page,'params'=>\$params])}");
        $this->assign("admin_js", "__STATIC__/js/admin.js");//引入admin.js文件
    ...
    protected function findViews($parm, $controllers, $fields)
    {
        ...
        $this->assign('controllers_name', $this->model->uncamelize($controllers));//控制器
        $this->assign("admin_js", "__STATIC__/js/admin.js");//引入admin.js文件
        $this->assign("url_find_id", "{:url('find',['id'=>\$id,'params'=>\$params])}");//编辑id
    ...
    protected function formViews($parm, $controllers, $fields)
    {
        $this->assign('fields', array_filter($fields, static function ($item) {
            ...
            //隐藏字段
            $not_show = ['id', 'create_time', 'update_time', 'delete_time', 'list_order', 'is_recommend', 'status'];
            if (in_array(strtolower($item['COLUMN_NAME']), $not_show)) {
                return false;
            }
            return true;
     ...
     * @return array|false|string|string[]
     */
    public function ApiControllerViews($parm, $controllers, $fields, $table_result_name, $keyword = 'id')
    {
...
EOF;
        ...
        $content = str_replace(
            [
                ...
                '%success%', '%table_result_name%', '%api_function_name%', '%get_list%', '%keyword%', '%ValidateName%'],
            [
                ...
                $api_function_name, $get_list, $keyword, $ValidateName],
            $content
        );
