...
class AuthController extends RestBaseController
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getUserInfoById($user_id, $where = null)
    {
        $MemberInit = new \init\MemberInit();//用户管理
        $map        = [];
        $map[]      = ['id', '=', $user_id];
        $result     = $MemberInit->get_find($map);
        return $result;
    }


    /**
     * 根据toekn获取用户user_info   注:同步MemberModel
     * @param $token
     * @param $where openid为空 条件筛选[]
     * @return array|mixed
     * @throws \think\db\exception\DataNotFoundException
    ...
    public function getUserInfoByToken($token)
    {
        ...
        $map2      = [];
        $map2[]    = ['token', '=', $token];
        $map2[]    = ['expire_time', '>', time()];
        $userToken = Db::name("user_token")->where($map2)->find();
    ...
    protected function add_log()
    {
        ...
        foreach ($files as $file) {
            if ($file != '.' && $file != '..') {
                ...
                if ($fileLastModified < $deleteDate) unlink($filePath);
            }
        }

        // 写入文件中 & 保存前 10 天的日志
        $filename  = $logDirectory . date('Y-m-d') . ".log";
